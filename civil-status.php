...
<body id="kt_body" class="app-blank bgi-size-cover bgi-attachment-fixed bgi-position-center bgi-no-repeat" data-kt-app-page-loading-enabled="true" data-kt-app-page-loading="off">
                    ...
                    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
                        <div class="d-flex flex-column flex-column-fluid">
                            <div id="kt_app_content" class="app-content flex-column-fluid">
                                <?php 
                                    if($newRecord){
                                        require_once './app/Views/Page/civil-status/new.php';
                                    }
                                    else if(!empty($detailID)){
                                        require_once './app/Views/Page/civil-status/details.php';
                                    }
                                    else if(isset($_GET['import']) && !empty($_GET['import'])){
                                        require_once './app/Views/Page/import/import.php';
                                    }
                                    else{
                                        require_once './app/Views/Page/civil-status/index.php';
                                    }
                                ?>
                            </div>
                        </div>
    ...
    <?php
        $version = rand();
        $scriptFile = './assets/js/page/civil-status/index.js';

        if ($newRecord) {
            $scriptFile = './assets/js/page/civil-status/new.js';
        }
        else if (!empty($detailID)) {
            $scriptFile = './assets/js/page/civil-status/details.js';
        }
        else if (isset($_GET['import']) && !empty($_GET['import'])) {
