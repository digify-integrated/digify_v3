...
document.addEventListener('DOMContentLoaded', () => {
    const displayDetails = async () => {
        const transaction       = 'fetch job position details';
        const page_link         = document.getElementById('page-link')?.getAttribute('href') || 'apps.php';
        const job_position_id   = document.getElementById('details-id')?.textContent.trim();

        try {
            resetForm('job_position_form');
            ...
            if (data.success) {
                document.getElementById('job_position_name').value = data.jobPositionName || '';
            }
    ...
    attachLogNotesHandler('#log-notes-main', '#details-id', 'job_position');
    displayDetails();

    $('#job_position_form').validate({
        rules: {
            job_position_name: { required: true }
        },
        messages: {
            job_position_name: { required: 'Enter the display name' }
        },
        ...
        submitHandler: async (form, event) => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Save job position failed with status: ${response.status}`);
                }
