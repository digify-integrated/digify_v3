...
document.addEventListener('DOMContentLoaded', () => {
    const displayDetails = async () => {
        const transaction   = 'fetch language details';
        const page_link     = document.getElementById('page-link')?.getAttribute('href') || 'apps.php';
        const language_id   = document.getElementById('details-id')?.textContent.trim();

        try {
            resetForm('language_form');
            ...
            if (data.success) {
                document.getElementById('language_name').value = data.languageName || '';
            }
    ...
    attachLogNotesHandler('#log-notes-main', '#details-id', 'language');
    displayDetails();
    ...
    $('#language_form').validate({
        ...
        messages: {
            language_name: { required: 'Enter the display name' }
        },
        ...
        submitHandler: async (form, event) => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Save language failed with status: ${response.status}`);
                }
