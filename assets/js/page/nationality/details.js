...
document.addEventListener('DOMContentLoaded', () => {
    const displayDetails = async () => {
        const transaction       = 'fetch nationality details';
        const page_link         = document.getElementById('page-link')?.getAttribute('href') || 'apps.php';
        const nationality_id    = document.getElementById('details-id')?.textContent.trim();

        try {
            resetForm('nationality_form');
    ...
    attachLogNotesHandler('#log-notes-main', '#details-id', 'nationality');
    displayDetails();
    ...
    $('#nationality_form').validate({
        ...
        messages: {
            nationality_name: { required: 'Enter the display name' }
        },
