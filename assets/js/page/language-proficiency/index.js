...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (language_proficiency_id.length === 0) {
            showNotification('Deletion Multiple Language Proficiencys Error', 'Please select the language proficiencys you wish to delete.', 'error');
            return;
        }

        const result = await Swal.fire({
            title: 'Confirm Multiple Language Proficiencys Deletion',
            text: 'Are you sure you want to delete these language proficiencys?',
            icon: 'warning',
