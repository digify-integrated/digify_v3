...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', async (event) => {
        ...
        const blood_type_id     = Array.from(document.querySelectorAll('.datatable-checkbox-children'))
                                    .filter(el => el.checked)
                                    .map(el => el.value);

        if (blood_type_id.length === 0) {
            showNotification('Deletion Multiple Blood Types Error', 'Please select the blood types you wish to delete.', 'error');
            return;
        }
        ...
        try {
            ...
            blood_type_id.forEach(id => formData.append('blood_type_id[]', id));
