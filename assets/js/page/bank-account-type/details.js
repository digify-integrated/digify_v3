...
document.addEventListener('DOMContentLoaded', () => {
    const displayDetails = async () => {
        ...
        try {
            ...
            formData.append('bank_account_type_id', bank_account_type_id);
            ...
            if (data.success) {
                document.getElementById('bank_account_type_name').value = data.bankAccountTypeName || '';
            }
    ...
    attachLogNotesHandler('#log-notes-main', '#details-id', 'bank_account_type');
    displayDetails();
    ...
    $('#bank_account_type_form').validate({
        ...
        submitHandler: async (form, event) => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Save bank account type failed with status: ${response.status}`);
                }
