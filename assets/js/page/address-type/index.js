...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initializeDatatable({
        ...
        transaction: 'generate address type table',
        columns: [
            { data: 'CHECK_BOX' },
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (address_type_id.length === 0) {
            showNotification('Deletion Multiple Address Types Error', 'Please select the address types you wish to delete.', 'error');
            return;
        }
        ...
        try {
            ...
            address_type_id.forEach(id => formData.append('address_type_id[]', id));

            const response = await fetch('./app/Controllers/AddressTypeController.php', {
                method: 'POST',
                body: formData
            });
